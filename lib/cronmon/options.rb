    ...
    class TOMLSource
      ...
      def load
        if @path and File.exists?(@path.to_s)
          result = TOML.parse(IO.read(@path.to_s))
        end
        result || {}
      end
  ...
  class Options < OpenStruct
    ...
    def files
      if(@files.nil? or @files.empty?)
        @files = ["#{File.join(File.dirname(__FILE__), "defaults.toml").to_s}",Cronmon::AUTH_CONFIG_FILE,Cronmon::TASKS_CONFIG_FILE,Cronmon::SETTINGS_CONFIG_FILE,File.expand_path("~/.cronmon.toml")]
      end
      @files
    end
    ...
    def to_hash
      ...
      marshal_dump.each do |k, v|
        result[k] = v.instance_of?(Cronmon::Options) ? v.to_hash : v
      end
      result
    end
